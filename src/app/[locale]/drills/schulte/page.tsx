...
import SchulteGame from '@/drills/schulte/SchulteGame';

export function generateStaticParams() {
  return routing.locales.map((locale) => ({ locale }));
}

export default async function SchultePage({
  params,
}: {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-2 pt-[max(1rem,env(safe-area-inset-top))]">
      <DrillHeader title={td('schulte')} backLabel={tc('back')} />
      <DrillGate title={td('schulte')} rule={tRule('schulte')}>
        <SchulteGame />
      </DrillGate>
    </main>
  );
