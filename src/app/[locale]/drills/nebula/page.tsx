...
import NebulaGame from '@/drills/nebula/NebulaGame';
...
export default async function NebulaPage({
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-2 pt-[max(1rem,env(safe-area-inset-top))]">
      <DrillHeader title={tg('item.nebula.name')} backLabel={tc('back')} />
      <DrillGate title={tg('item.nebula.name')} rule={tRule('nebula')} survival>
        <NebulaGame />
      </DrillGate>
    </main>
  );
