...
export default async function Page({
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-2 pt-[max(1rem,env(safe-area-inset-top))]">
      <DrillHeader title={tg('item.fingerDance.name')} backLabel={tc('back')} />
      <DrillGate title={tg('item.fingerDance.name')} rule={tRule('fingerDance')}>
        <MotorGame config={MOTOR_CONFIGS.fingerDance} />
      </DrillGate>
    </main>
  );
}
