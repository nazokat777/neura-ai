...
import AudioMemoryGame from '@/drills/audio/AudioMemoryGame';
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-2 pt-[max(1rem,env(safe-area-inset-top))]">
      <DrillHeader title={tg('item.audioMemory.name')} backLabel={tc('back')} />
      <DrillGate title={tg('item.audioMemory.name')} rule={tRule('audioMemory')} survival>
        <AudioMemoryGame />
      </DrillGate>
    </main>
  );
